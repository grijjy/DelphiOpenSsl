...
type
  ...
  public
    ...
    class function Verify_RSASHA256(const AHeader, APayload, ASignature: TBytes; const APublicKey: TBytes): Boolean;
    ...
    { Creates an HMAC SHA1 hash of the provided data
      ...
      Returns:
        String containing the hash of the data and key }
    class function HMAC_SHA1(const AKey, AData: RawByteString): TBytes; static;
  public
...
class function TgoOpenSSLHelper.RandomBytes(const ASize: Integer): TBytes;
begin
...
class function TgoOpenSSLHelper.RandomString(const ACharset: String; const ASize: Integer): String;
var
  I: Integer;
  Bytes: TBytes;
begin
...
class function TgoOpenSSLHelper.RandomString(const ASize: Integer): String;
const
...
class function TgoOpenSSLHelper.RandomChars(const ASize: Integer): String;
const
...
class function TgoOpenSSLHelper.RandomLowerString(const ASize: Integer): String;
const
...
class function TgoOpenSSLHelper.RandomLowerChars(const ASize: Integer): String;
const
...
class function TgoOpenSSLHelper.RandomDigits(const ASize: Integer): String;
const
...
class function TgoOpenSSLHelper.Sign_RSASHA256(const AData: TBytes; const APrivateKey: TBytes;
...
class function TgoOpenSSLHelper.Verify_RSASHA256(const AHeader, APayload, ASignature: TBytes; const APublicKey: TBytes): Boolean;
const
...
class function TgoOpenSSLHelper.HMAC_SHA256(const AKey, AData: RawByteString): String;
const
  EVP_MAX_MD_SIZE = 64;
var
...
class function TgoOpenSSLHelper.HMAC_SHA1(const AKey, AData: RawByteString): TBytes;
const
  EVP_MAX_MD_SIZE = 20;
var
  MessageAuthCode: PByte;
...
class function TgoOpenSSLHelper.CertToPEM(const ACertificates: array of PX509; const APrivateKey: PEVP_PKEY;
  out ACertificatePEM, APrivateKeyPEM: TBytes): Boolean;
var
  Certificate: PX509;
  BIOCert, BIOPrivateKey: PBIO;
  DERCert, DERPrivateKey: TBytes;
...
begin
  ...
  begin
    ...
    try
      ...
      Read := BIO_read(BIOCert, DERCert, Pending);
      if Read > 0 then
      begin
        SetLength(DERCert, Read);
        Base64 := TBase64Encoding.Create(64, #10);
        try
          ACertificatePEM := ACertificatePEM + TEncoding.ANSI.GetBytes(
  ...
  try
    ...
    Read := BIO_read(BIOPrivateKey, DERPrivateKey, Pending);
    if Read > 0 then
    begin
      SetLength(DERPrivateKey, Read);
      Base64 := TBase64Encoding.Create(64, #10);
      try
        APrivateKeyPEM := TEncoding.ANSI.GetBytes(
          '-----BEGIN RSA PRIVATE KEY-----' + #10 +
           Base64.EncodeBytesToString(DERPrivateKey) + #10 +
...
class function TgoOpenSSLHelper.CreateCertRequest(const ACountry, AState, ALocality, AOrganization, AOrgUnit, ACommonName: String;
...
class function TgoOpenSSLHelper.SetRandomSerial(const ACertificate: PX509): Boolean;
var
	Buffer: TBytes;
	Bignum: PBIGNUM;
...
begin
  ...
  Buffer := TgoOpenSSLHelper.RandomBytes(20);
  ...
  try
    BN_bin2bn(Buffer, Length(Buffer), Bignum);
...
class function TgoOpenSSLHelper.CreateKeyPair_CA(const ACertificateCA: PX509; const APrivateKeyCA: PEVP_PKEY;
  const ACountry, AState, ALocality, AOrganization, AOrgUnit, ACommonName: String;
  const AServerName: String; const AExpiresDays: Integer;
...
class function TgoOpenSSLHelper.CreateSelfSignedCert_X509(
  const ACountry, AState, ALocality, AOrganization, AOrgUnit, ACommonName: String;
  const AServerName: String; const AExpiresDays: Integer;
  out ACertificate, APrivateKey: TBytes): Boolean;
var
  Certificate: PX509;
  PrivateKey: PEVP_PKEY;
  Name: PX509_NAME;
...
class function TgoOpenSSLHelper.CreateSelfSignedCert_X509CA(const ACertificateCA, APrivateKeyCA: TBytes; const APassword: String;
  const ACountry, AState, ALocality, AOrganization, AOrgUnit, ACommonName: String;
  const AServerName: String; const AExpiresDays: Integer;
  out ACertificate, APrivateKey: TBytes): Boolean;
var
